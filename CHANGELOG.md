...
### 正式版发布轮（2026-09-25，转 1.0.0）

- **版本号 `0.1.0-alpha.10` → `1.0.0`**：功能零改动（仅版本号与文档）。
  不沿用 `0.1.0` 因其已被 Alpha 之前的首个标记版本占用（tag `v0.1.0`，提交 `8e6bd04`，
  已推送不可移动），正式版顺延为 `1.0.0`。
- **验收闭合口径**：X1/X3/X3c 同窗口真机应用验证与 W1–W5/X1–X3c 整体观感回执
  已由使用者确认（2026-09-25 回执「通过」）；alpha.8/9/10 的 Release 网页回执补记
  按使用者决定跳过，不再挂账。收窄声明不变：未签名、仅开发机 Windows x64 验证、
  1.18.29 完整验证 + 结构验证通道。
- 发布说明 `docs/release-notes-1.0.0.md`（发布物表已随候选登记回填，状态已转「已发布」）。
- **GitHub Release `v1.0.0` 已创建并发布**（2026-09-25，使用者指示「创建 GitHub Release」，
  经 REST API 由 wb 侧执行——对本仓「Release 上传归 jc」先例的一次显式授权偏离）：
  zip 与哈希侧车两条资产上传成功、匿名下载核验通过。执行中查实**仓库实为公开**
  （与 9.4–9.13 各节「私有仓库」表述不符，翻转时点不明），使用者拍板接受公开。
  详见 `docs/alpha-acceptance.md` §9.14 Release 回执补记。

### 观感修复与功能回合 W1–W5（2026-09-24/25，随下一候选生效）
...
- **W3（交互行为变化）**：可读性门软化——对比度未达标从按钮级硬拦改为应用前显式确认放行
  （仅绕对比度门；正文对比度底线硬闸不动，正文不达标连预览都没有）。
  IPC 新增 `StageThemeInput.allowContrastOverride`（非布尔按 `INVALID_PARAMS` 拒），
  无放行照旧拒且错误 detail 带逐项清单；放行时 `contrastOverride` 贯通
  staged 记录→应用→事务落证据。确认框焦点默认「取消」、确认钮红色、不记住选择。
- **W4（功能复活）**：状态栏左侧「启动 OpenCode」按钮回归（运行中显示「打开 OpenCode」）。
  启动方式按 W4a 真机探测定案改用 ShellExecute 等价路径（`cmd /c start "" "<exe>"`，与双击同路径），
  实测会话完整恢复；F1 时代 `spawn` 直拉「加载不了对话」的问题不再存在。
  安全边界不变：renderer 只传 targetId，exe 路径由主进程按适配器解析。
  探测证据 `handoff/visual-fix-plan-2026-09-24/W4A-EVIDENCE.md`，定论已写入 `docs/compatibility.md`。
- **W5（预览呈现变化）**：模拟预览的背景模糊半径按 mock 容器宽/假定真机窗宽（1280px）
  缩比折算（inset 随动），此前同像素模糊在小容器里相对过糊；写入真机的参数与注入层零改动。
  残余差距（缩略图分辨率、cover 裁切区）属缩比渲染物理极限，如实记录不硬凑。
  诊断清单与并排目检见 `handoff/visual-fix-plan-2026-09-24/W5-EVIDENCE.md`。

### 观感修复第二轮 X1–X3（2026-09-25，随 alpha.9 生效）

- **X1（默认观感变化）**：下拉菜单/弹层/提示从与 dialog、输入框**共用**的半透明面板组里拆出，
  单列为**不透明深色面**（`menuSurfaceColor`：深色模式把面板色再加深一档、浅色模式轻微加深，
  均 alpha=1），文字经 `ensureContrast` 兜底对新底色明暗两模式都 ≥4.5；边框保持 0.9。
  dialog 与 prompt-input 留在半透明组不动（其半透明是 W1/W2 定过的观感）。mock 预览菜单面同步为
  同一函数输出。计划见 `handoff/visual-fix-r2-plan-2026-09-25/PLAN.md`。
- **X2（界面信息架构）**：「检查」选项卡里的可读性面板整体收成**一行结论条**
  （标题 + ✓/✗ 结论 + 余量最差一项），点结论条展开整卡；底色、28 项明细与
  范围/采样说明的既有结构原样保留在展开区。默认态按结论定：未达标**默认展开**
  （问题必须被看到），全达标默认收起。报告数据与判定逻辑零改动。
  计划见 `handoff/visual-fix-r2-plan-2026-09-25/PLAN.md`。
- **X3（默认观感变化 + 取证）**：封面（空会话首页）标语「Build anything」是官方
  `.text-20-medium.text-text-strong`（真机取证坐实：整份官方 bundle 仅此一处），无底色直接浮在壁纸上、
  在深/浅壁纸上易发虚。注入层给它加一层**与主题背景同源的柔和 text-shadow**（不加底框，保官方浮层观感）——
  该锚点只命中封面标语、天然「封面上下文限定」，零误伤会话内。**封面输入框加深本轮不做**：真机取证显示
  封面与会话内是同一套 `session-prompt-dock > prompt-input-v2`、DOM 层无 cover/session 区分属性，
  `:has()` 子树锚点静态无法确认命中，按「不猜」留待真机窗口由 jc 坐实结构后再定。
  取证与定案见 `handoff/visual-fix-r2-plan-2026-09-25/X3-EVIDENCE.md`。

### 封面固定不透明 X3c（2026-09-25，随 alpha.10 生效）
