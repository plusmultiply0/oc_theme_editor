...
const sourceImageHash = '545a6e0a1aeb4ba28c7616a813ac199add7d7c674b17caf4b6d27db23921aa55';
const expected = '7d36e0a78e2cd1aadd046230e38c0085313c4b87aee86f96e97c6914491fd7cd';
const stylesheet = path.join(base, 'pastel-theme.css');
const staged = path.join(base, 'app.pastel-v2.asar');
const record = path.join(base, 'pastel-v2-manifest.json');
...
function nativeRead(file, cssHash, imageHash) {
  ...
  const code = `const fs=require('fs'),crypto=require('crypto'),assert=require('assert/strict');
    ...
    assert.equal(hash(fs.readFileSync(root+'pastel-background.png')),${JSON.stringify(imageHash)});
...
function stage() {
  ...
  for(const m of css.toString().matchAll(/url\(\s*([^)]*)\s*\)/gi)) assert.equal(m[1].trim().replace(/^['"]|['"]$/g,''),'./pastel-background.png');
  const changes = new Map([[p+'snow-theme.css',css],[p+'pastel-background.png',image]]);
  fs.writeFileSync(staged,pack(current,changes),{flag:'wx'});
  const after=load(staged);
  assert.equal(current.entries.size + 1,after.entries.size);
...
function apply() {
  ...
  assert.equal(prev.patchedHash,expected);
  assert.equal(digest(prev.backup),prev.originalHash);
  const dir=path.join(base,'backups','pastel-'+new Date().toISOString().replace(/[:.]/g,'-'));
  ...
  try {
    ...
    prev.revisions ||= [];
    prev.revisions.push({backup:data.previousBackup,hash:expected,created:new Date().toISOString(),theme:'arknights-warm-gold'});
    prev.patchedHash=data.patchedHash; prev.imageHash=data.imageHash;
    prev.themeRevision=4;prev.themeName='pastel-black-text';prev.updated=new Date().toISOString();
...
function restore() {
  ...
  data.status='restored-arknights-theme';writeJSON(record,data);
  console.log('Previous Arknights wallpaper and palette restored; original backup still retained.');
}
