...
**2026-09-21 轮（alpha.3，structural 通道真机首跑）：应用链通过；发现并登记一处阻塞级缺陷**。

- **首跑拦停（缺陷实证）**：apply 报 `ARCHIVE_CORRUPT`「目标与首次接管快照不一致
  （1075 条）」，安装未被修改。根因：实机 OpenCode 已自动更新 1.18.29→1.18.31，
  而首次接管基线按**安装实例**键控、不随版本分键（layout.ts `originalDir` +
  apply.ts `resolveBaselineGate`）——**structural 通道放行与 apply 基线硬门互不相交**：
  凡「旧版本接管过 → 自动更新」的机器，apply 恒被拒；且提示语建议的「恢复到首次接管时」
  等于把旧 asar 局部降级回新 exe 目录，更新后复发循环。**工具没有「重新接管/重建基线」
  入口**——登记缺陷，修复立后续轮（不进 alpha.3）。
- **处置（jc 当次授权，方案「归档旧接管」）**：只读前提核验通过（locks 空、
  25 条历史事务全终态 22 applied/2 failed、OpenCode.exe 未运行）后，
  `backups/original` → `original.archived-20260921`（1.18.29 快照 `1c53ca24…`、
  旧 baseline、meta 原样保留，可逆；界面不再列旧「首次接管」入口）。
- **重放成功（新基线按 1.18.31 建立）**：新接管快照
  `original-2026-09-21T13-17-43-560Z.asar`（version 1.18.31，接管前指纹
  `9290532a…`，144,345,374 B）；事务 `op-20260921T131700558Z-z4uk5m` 状态
  `applied`。应用后只读 probe 复核：注入标记齐全（检查 5「变更集合归属」PASS、
  锚点唯一 PASS），归档指纹 `16f610d8…`（144,298,107 B）。
- **回执边界（如实）**：jc 回执「成功应用了」，属**整体回执**——structural 徽章
  配色、确认框逐项文案、应用后重启的画面观感**未逐项回执**，A5 视觉走查仍待补；
  本轮仅证明应用链（含确认框必经路径）与数据面正确。

**[已修 2026-09-22（baseline-drift 轮，本缺陷的正式修复）]** 上面登记的
「接管后自动更新致基线恒拒、无重新接管入口」已在 main 修复：**不删基线比对**
（防篡改职责保留），改为差异分类裁决——`compareWithBaseline` 产出结构化差异，
`classifyDrift` 五条件（版本变化/锚点唯一/变更集合干净/差异规模达整体替换量级
50% 或 package.json 变化且 ≥100 条/unpacked 原生模块路径集合一致）**同时满足**
才自动重新接管（重建基线、旧基线按 `baseline.v{N}.json` 序号归档永不删除、
操作记录 `rebaselined`+`baselineFromVersion` 留证据、进度事件可见告知）；
任一不过维持拒绝，文案区分「疑似第三方改动」与「疑似官方更新但未达自动放行条件」。
提交：`c957272`（B1 实现）+ `3ee6f0c`（B2 正负例，单测 24 + 端到端 5）+ `bca0071`（B3 收尾）。
门禁：typecheck/lint 0、单测 366、集成 188 全绿。**边界如实**：判定是结构级、
非密码学校验；修复**不在已发布的 alpha.4**，随下一候选生效；「更新后重应用放行」
在 jc 真机（1070 条场景）的复验待当次授权后进行，agent 侧门禁绿 ≠ 真机复验完成。

**[真机复验通过 2026-09-23（jc 当次授权，1.18.31→1.18.32 场景）]** 上面「待真机复验」项已闭环：
应用前实机为 1.18.32 supported（基线是 09-21 按 1.18.31 建的 legacy 裸数组，恰好额外打穿
legacy 回推路径），locks 空、最近事务终态、OpenCode 已完全退出。经 `tools/live-cli.cjs apply
--confirm-structural`（CLI 补的确认通道，`79057d7`）重应用：进度事件如实出现
「检测到目标已更新至 1.18.32，已自动更新基线（旧基线归档留存）」→ applied
（`op-20260923T075039977Z-1qnvtm`，指纹 e711a876…→33a6cadc…）。应用后只读核验：
...
**[观感回执 2026-09-24（界面信息架构重整 T1–T5）]** jc 真机回执「通过」——按惯例属
**整体回执**（非逐项）：顶栏瘦身+目标芯片 tooltip、启动按钮入状态栏最左、右栏
[检查][恢复] 选项卡、可读性概要化（明细折叠）、恢复区精简（空态一行+长说明折叠）
观感登记为已确认。改动纯呈现层（服务/IPC/预览层零改动），提交链
`475c295`→`e6500d5`（README 同步 `09486ca`），门禁 lint/typecheck 0、单测 366、
e2e 16、e2e:electron 38 全绿（计划与验收口径见 `handoff/ui-restructure-plan-2026-09-24/PLAN.md`）。
**该批改动不在已发布的 alpha.5 包内**，随下一候选生效；alpha.6 发布轮开工与否归 jc 拍板。
...
- **tag / Release**：tag `v0.1.0-alpha.6` 已打在候选登记 HEAD `819299c` 并推送 `origin`
...
- **tag / Release**：tag `v0.1.0-alpha.7`（annotated）已打在候选登记 HEAD `eecea37` 并推送
  ...
  **2026-09-24 jc 网页回执（页面 URL）**：
  `https://github.com/plusmultiply0/oc_theme_editor/releases/tag/v0.1.0-alpha.7`。
  附件可见性以该回执为准（私有仓库，wb 侧无匿名核验通道，同 9.4–9.9 口径）；
  发布说明状态行随之转「已发布」。
- 至此 alpha.7 发布轮 agent 侧与上传侧均闭环。**遗留**：新界面整体观感回执（本轮特有
...
### 9.12 候选登记（2026-09-25，alpha.9，渠道：GitHub 私有仓库 Release）

- **签核口径**：jc 指令为执行 `handoff/visual-fix-r2-plan-2026-09-25/PLAN.md` 的 X1–X3
  （2026-09-25 起，一项一提交，顺序 X2→X3a 取证→X1→X3b，完成后随下一候选 alpha.9
  发布、不动已登记的 alpha.8）。本轮增量源码=观感修复第二轮 X1–X3
  （`9f45251` X2 面板折叠、`3243e0d` X3a 取证入档、`88ad428` X1 菜单不透明面、
  `9283170` X3b 封面标语描边）+ 本发布轮文档。本轮**纯观感与界面收纳**：
  判定、事务、备份、恢复逻辑与 IPC 契约零改动；X1/X3 只动注入 CSS 呈现层，
  X2 只动本工具界面折叠呈现。**登记前缺口（比 9.10/9.11 更深一档，如实标注）**：
  PLAN 要求的 X1/X3 **同窗口真机应用验证未做**——jc 未在场、本轮未获真机写入授权，
  只完成了 X3a 只读取证；菜单不透明面与标语描边的真机观感、会话内输入框无回归
  三项验证留待 jc 真机窗口（任务 #76）。整体观感回执同样未闭合（同款缺口）。
  不宣称 A9 全签核（A6 仍跳过）。
- **本机验收**：typecheck / lint / unit（22 文件 386 条）/ integration（16 文件 190 条）
  / e2e（16 条，含 X2 折叠态适配）全绿；X1 不透明面文字对比度经单测自证明暗两模式
  ≥4.5；X2 默认态经截图目检（临时实例，截图含本机路径不入库）；封面输入框加深
  按 X3-EVIDENCE 定案 2 **本轮未做**（不猜 `:has()` 锚点），观感同 alpha.8。
- **链运行**：`npm run release:build` 由 agent 会话后台跑，**首跑即一次跑满 12 步全绿、
  无拦停轮**，末行 `ALL_GREEN buildId=20260925050329-f2434b8-057575`。`smoke:gui`
  （空参数，25.3s）、`test:e2e:electron`（25.9s）均真实通过。发布级只读终检 18 项
  0 失败，`RELEASE_GREEN`、`publishable=true`。
- **候选身份**：buildId `20260925050329-f2434b8-057575`，来源提交 `f2434b8`（版本号
  bump + 发布说明草案），`packMethod: electron-builder`，可重复构建。
...
- **真机状态**：本机 OpenCode 安装自 9.11 的 restore（`op-20260925T024944346Z-lcyqr7`）
  后保持原状，本轮**未做任何真机写入**；X3a 取证为只读（解包读官方 bundle，不落盘目标）。
- **登记**：`release-checklist.md` §1 权威入口块换为上述 buildId 与哈希，§2.11 把 alpha.8
  转历史（其 manifest/哈希原样保留，并注明 X1–X3 不在 alpha.8 包内），§3 门禁表按本轮
  build-record 改写；README 候选指针同步（当前候选 buildId/目录两处 + W1–W5 条目改
  「随 alpha.8 分发」+ X1–X3 随 alpha.9 分发新增条目）。侧车 `.zip.sha256.txt`
  生成于登记后、分发前（不入库，同先例）。
  `doc-candidate-entry.cjs --manifest candidate-20260925050329-f2434b8-057575/
  candidate-manifest.json --check` 得 `DOC_ENTRY_OK`（文档、manifest、磁盘三方一致）。
- **tag / Release**：GitHub pre-release 由 jc 在网页创建（本机无 gh CLI，同 9.4–9.11 先例），
  **wb 侧不上传、不宣称已发布**，待 jc 网页回执后补记页面 URL 并将发布说明状态行转
  「已发布」。**本节登记时不宣称 A9 签核**，远程可见性以 jc 网页回执为准。
  **tag 落账（同日补记）**：tag `v0.1.0-alpha.9`（annotated）已打在候选登记 HEAD `610cfdc`
  并推送 `origin`（`* [new tag]`，注释如实标注「X1/X3 同窗口真机应用验证与整体观感回执
  归使用者、登记前未闭合；封面输入框加深按取证定案本轮未做」口径）。
- 至此 alpha.9 发布轮 agent 侧闭环（候选登记 + tag 已推），上传待 jc。**遗留**：
  X1/X3 同窗口真机应用验证（本轮特有缺口，登记前未闭合，如实保留）、
...
- **tag / Release**：GitHub pre-release 由 jc 在网页创建（本机无 gh CLI，同 9.4–9.12 先例），
  ...
  **tag 落账（同日补记）**：tag `v0.1.0-alpha.10`（annotated）已打在候选登记 HEAD `7c4b193`
  并推送 `origin`（`* [new tag]`，注释如实标注「X1/X3/X3c 同窗口真机应用验证（#76，含
  大字随图自适应新增项）与整体观感回执归使用者、登记前未闭合；链首跑拦停、修复后
  第二跑跑满全链」口径）。
- 至此 alpha.10 发布轮 agent 侧闭环（候选登记 + tag 已推），上传待 jc。**遗留**：
  ...
  **同日后续更正（见 9.14）**：使用者回执「通过」并明确覆盖真机项（上文第 1、2 项闭合）；
  alpha.8/9/10 的 Release 网页回执补记按使用者决定**取消**（第 3 项不再挂账）。

### 9.14 正式版 1.0.0 候选登记与发布（2026-09-25，渠道：GitHub Release——仓库实为公开，见 Release 回执补记）

- **签核口径**：jc 决定跳过 alpha.8/9/10 的 Release 网页回执补记（不再挂账），并对
  W1–W5/X1–X3c 给出总体回执「**通过**」；经追问确认该回执**覆盖 #76 同窗口真机应用验证**
  （菜单不透明面、封面两块固定不透明且随滑杆纹丝不动、大字颜色随明暗壁纸自适应、
  会话内输入框无回归）。至此 alpha 轮全部验收缺口闭合。jc 指示发布正式版；
  版本号原拟 `0.1.0`，因 tag `v0.1.0` 已被 Alpha 前首个标记版本占用（`8e6bd04`，
  已推送不可移动），经选拍板改 **`1.0.0`**。本候选相对 alpha.10 **功能零改动**
  （增量源码 = `0160900` 版本号 bump + 发布说明/CHANGELOG + 本发布轮文档 +
  `be0e1e7`/`05e43b2` README「打开换肤助手」exe 相对路径入口与使用者预置的
  「效果展示」截图段落账），判定/事务/备份/恢复/IPC 契约与 alpha.10 完全一致。
- **本机验收**：typecheck / lint / unit（22 文件 389 条）/ integration（16 文件 190 条，
  含 Electron fixture 38/38）/ e2e（16 条）全绿；发布说明 `docs/release-notes-1.0.0.md`
  按收窄口径撰写（未签名、仅开发机 Windows x64 验证、1.18.29 完整验证 + 结构验证通道、
  A6 按决定跳过），已知限制不含任何「验证未做」项——真机与观感两项已闭合，如实移除。
- **链运行**：`npm run release:build` 由 agent 会话后台跑，**首跑即一次跑满 12 步全绿、
  无拦停轮**，末行 `ALL_GREEN buildId=20260925081334-0160900-e4b156`。`smoke:gui`
  （空参数，20.9s）、`test:e2e:electron`（23.3s）均真实通过。发布级只读终检 18 项
  0 失败，`RELEASE_GREEN`、`publishable=true`。
- **候选身份**：buildId `20260925081334-0160900-e4b156`，来源提交 `0160900`（版本号
  bump + 发布说明草案），`packMethod: electron-builder`，可重复构建。
  ...
  （1064 条目 / unpacked 7）。zip 127.2 MB / 82 条目。
- **真机状态**：本机 OpenCode 安装自 9.11 的 restore（`op-20260925T024944346Z-lcyqr7`）
  后保持原状，本发布轮**未做任何真机写入**（#76 验证由使用者自行完成，证据归其持有）。
- **登记**：`release-checklist.md` §1 权威入口块换为上述 buildId 与哈希（标题转
  「当前候选（正式版）」），§2.13 把 alpha.10 转历史（manifest/哈希原样保留，注明
  X3c 与 coverText 修复都在其包内、1.0.0 相对其零改动），§3 门禁表按本轮 build-record
  改写并落验收闭合口径；README 三处候选指针同步 + 「打开换肤助手」路径行随新候选目录
...
- **tag / Release**：tag `v1.0.0`（annotated）已打在候选登记 HEAD `e85d1df` 并推送
  ...
  口径）。
  **Release 回执补记（同日，后续追加）**：使用者指示「创建 GitHub Release」，wb 侧据此
  经 GitHub REST API（本机无 gh CLI；token 走 `git credential fill` 内存内取用、不落盘
  不打印）创建 Release 并上传分发物——这是对本仓「Release 上传归 jc」先例的**一次
  显式授权偏离**。执行中查实：仓库为**公开**（鉴权 `private=false` + 匿名 api/Release
  页均 200），与 9.4–9.13 各节「私有仓库/受控分发」前提不符（翻转时点不明，历史条目
  不改写、以本条实况为准）；据实上报后使用者拍板「接受公开，直接上传」。
  Release：https://github.com/plusmultiply0/oc_theme_editor/releases/tag/v1.0.0
  （正文 = 发布说明全文 + 分发物附注）。资产两条：zip（上传回执 size=133391490，与
  本地磁盘字节一致）与哈希侧车 `.zip.sha256.txt`。首传曾因 `upload_url` 模板
  `{?name,label}` 拼接缺陷被 400 拒两次，修复后第二跑 `UPLOAD_ALL_OK`。匿名核验：
  Release 页 200、zip Range 下载 206、侧车内容与本地逐字节一致。发布说明状态行已转
  「已发布」。
- 至此正式版发布轮 agent 侧**全闭环**（候选登记 + tag 已推 + Release 已发布、
  资产上传并匿名核验）。**遗留**：
  「启动后实发消息」核验、真机逐项视觉走查（P3 色块外观缺陷口径不变）、A6 干净环境
  （按决定跳过，平台声明已按窄口径写死）——均已在发布说明「已知限制」如实列出，
  不阻塞本版本登记与发布。alpha.8/9/10 的 Release 补记维持按决定跳过（不随本次授权
  扩大范围）。
