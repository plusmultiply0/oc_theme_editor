...
- **A8 距 GO 只剩一项流程**：「用户明确批准发布目标、版本与内容」——这是 jc 的签核动作，
  不是证据缺口。wb 侧建议：可刷 GO。
- **如实提醒**：上表 A1/A2 两行仍为「待执行」（人工核对项，从未执行）。§8.3 的
  「缺口收敛至 A5」指三类 NO-GO 理由，不含 A1/A2 人工核对；签核 GO 前 jc 需二选一——
  让我补做 A1/A2 核对，或在发布说明中如实声明其未做并视为豁免。
- 本文件不擅自刷 GO：结论改为「**待用户签核**」，由 jc 明确批准后生效。
