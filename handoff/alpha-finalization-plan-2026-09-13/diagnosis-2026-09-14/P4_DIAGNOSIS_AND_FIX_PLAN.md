...
- **持锁者证据（2026-09-15 纠偏：仍属「待证实」，不构成归因）**：本机
  `WinDefend` / `WdNisSvc` 均为 `Stopped`，而 **`QQPCRtp`（腾讯电脑管家实时防护）
  为 `Running`**。但**服务 Running 不能证明它持有目标文件锁**：既没有失败时刻
  的**目标文件句柄**证据，也没有与失败时间对应的**文件系统事件**证据。
  因此这里只能写「实时防护是**候选**因素」，**不得**写成已证实持锁者。
- **与删除护栏是两类机制（r3-S6 纠偏）**：`build` 步骤 `rmSync('out')` 被
  `SAFE_DELETE_BULK_CONFIRM_REQUIRED` 拦截，是**删除策略的审批拦截**；
  `rename` 的 `EPERM` 是**文件操作失败**。二者不得混为同一根因。

**判定**：属**环境相关访问失败**（可能与本机实时防护有关，**归因待证实**），
**不是产品缺陷**。验收项「EPERM/首次 apply 失败有具体错误码及诊断信息，
未靠关闭防护或吞异常放行」**已满足**：错误码 `FILE_LOCKED`、诊断信息明确、
未吞异常、未关闭防护。

**不采取的处置**（维持原约定）：不关实时防护、不改系统防护设置、不强杀进程；
本轮亦未删除任何被占用的旧日志/文件。

**删除的错误建议（2026-09-15 纠偏）**：~~将仓库根与 `%TEMP%` 加入电脑管家
信任区（或暂停实时防护）~~ —— 这类「扩大防护豁免 / 关闭防护」**不得**作为让
测试变绿的默认步骤。正确做法：保持防护，收集 `code`/`errno`/`syscall`/时间/
目标路径/前后 hash，有限次数重试；确需确定持锁者时，由有权限的操作者取得
**针对该文件的句柄或文件系统事件证据**并与失败时间对应，拿不到就保持「待证实」。
审批类阻塞单独登记，不与产品错误混写。
