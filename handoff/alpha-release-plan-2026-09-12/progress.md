...
## A5 授权与前置准备（2026-09-13，jc 授权；只做只读部分）

jc 已授权 A5，要求先给准备步骤。本次**只做只读采集**，未写入真实安装。

现场（详见 `A5-prep.md`）：目标归档 150,610,355 字节 / `17092e8e…` / 6996 条目；
本工具链接唯一、标记唯一、位于 head 内（只读核验全 OK）——说明真机当前已挂主题；
备份 `original` 1 条（`pristine=false`）+ `previous` 9 条。

重要预期：因 `pristine=false`，界面**不会**出现「恢复原版」入口，
A5 只能验「恢复上一主题」与「恢复到首次接管时」——与 A1 修正后的文档口径一致。

等待 jc：保存工作并**完全退出 OpenCode**（含托盘）后回一句，再由 wb 执行应用 → 重启 →
换图（JFIF/PNG/WebP）→ 恢复闭环与视觉走查。

## A6 与 LICENSE 的决定（2026-09-13，jc 拍板）

- **A6 跳过**：没有非开发环境机器/VM。因此发布声明不得声称已在干净环境验证，
  只写「已实测：开发环境」，平台声明按此收窄（A8 的 NO-GO 项之一无法消除）。
- **补 LICENSE**：新增仓库根目录 `LICENSE`（MIT，版权行 `Copyright (c) 2026 jc`），
  `package.json` 补 `author: jc`、`license: MIT`。
  注意：这两处是**元信息改动**，当前候选包是在改动前构建的，包内 package.json 不含 author；
  属元数据差异、不影响运行时，下次构建会同步。
