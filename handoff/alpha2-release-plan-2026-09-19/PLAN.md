...
| A2-4 | 本提交（记录随落账提交） | **完成（2026-09-19）**：jc 签核「提交 A2-4 的 tag 和 Release 三件套」；tag `v0.1.0-alpha.2`（注释含 buildId+zip SHA256，指向登记 HEAD `f7abd2f`）已推 origin；上传前复算 zip==侧车==权威块；Release 由 jc 网页发布（pre-release，私有仓库，本机无 gh CLI），附件 zip+`.zip.sha256.txt`（侧车先生成，alpha.1 偏差已纠）、正文为定稿 notes；可见性以 jc 回执「已发布」为准。落账 `docs/alpha-acceptance.md` §9.5 |
