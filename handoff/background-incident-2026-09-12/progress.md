...
前置：OpenCode 完全退出（进程检查为 0）；预检通过（1.18.29 supported、进程 idle、可写）。
使用 `tools/live-cli.cjs`（与 GUI 同一服务层与事务逻辑；产物为 release5 同源代码）。

| 步骤 | 结果 |
|---|---|
| 应用 A（用户壁纸 `80fbd6a9…`，浅色 / 遮罩 .59 / 面板 .31） | applied，归档 `2b75faf7…` |
| 应用 B（纯色识别图 `ae03d212…`） | applied，归档 `89748b46…` |
| 再应用 A | applied，归档回到 `2b75faf7…`（确定性一致） |

最终状态核验（`tools/verify-real-install.cjs --deep`，16 项 0 失败）：
图片 4,947,909 字节且哈希等于用户壁纸；HTML 链接恰好一个、标记唯一、在 head 内；
CSS 用 `html:root`、stronger 为 rgba、外壳限定规则存在；
**逐条完整性 6949 条通过、无 offset 冲突、1293 个脚本解析通过**（2776 个 ESM/TS 声明如实跳过）。

### 真机暴露的三个额外缺陷（已修，各一个提交）

1. **准备区清理阻塞结果**（`27438a3`）：递归删除 224MB/1260 文件的准备区被卡住约 25 分钟
   （CPU 停摆、文件数零进展、外部进程删除同样目录只要 5–14 秒），而安装早已换好。
   `await` 它会让界面一直停在 100%。改为后台执行 + 吞错，残留由启动时 `cleanAllStages` 处理；
   新增 `hooks.cleanup` 与回归（清理挂起时 apply 仍立即返回且安装已更新）。
2. **预检空间估算少算一份**（`da9aaaf`）：旧估算 3 份（495 MB 放行），实际峰值是 4 份
   —— 还有「解包出来的准备区目录」。真机因此在提交阶段 ENOSPC；安全机制正确中止
   （安装未被修改），但估算错了，改为 4 份 + 64MB。同批让 live-cli 启动时清理残留准备区。
3. **live-cli 启动清理无上界**（`0d47453`）：加 30s 上界；另让它遵守已有的
   `THEME_SWITCHER_NO_REGISTRY`（受限环境下注册表查询会被安全策略拦截）。

### 仍需 jc 完成

- **启动 OpenCode 肉眼验收**：首页、会话、侧栏、输入、菜单、旧/新布局是否都透出背景图；
  官方主题切换、明暗模式切换、重启后是否仍在；dialog/menu/tooltip 等 Portal 是否被破坏。
- 未做真机 DOM 探针（`probe-live-background.js`），未截真实窗口图。
- 本次未重新打包（源码在清理/估算/live-cli 三处有增量改动），发布前需 `npm run dist` 重建。
