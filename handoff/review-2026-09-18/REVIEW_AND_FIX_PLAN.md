...
| 8.4 步 3（A8 GO → 分发） | 证据侧缺口已清零，仅剩「用户明确批准发布目标、版本与内容」签核；签核前需对 A1/A2 人工核对补做或豁免（见 alpha-acceptance 9.2）。**[当日落定]** jc 指令「批准发布」→ **A8 = GO**（alpha-acceptance 9.3）：发布物三件磁盘核对、zip/exe/asar 哈希 2026-09-18 实算与权威块一致；随附 `.sha256.txt` 此前不存在（文档引用了从未生成的文件），GO 落账时补生成并如实登记为过程偏差；实际外发等 jc 指定渠道 |
